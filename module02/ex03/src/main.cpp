...
static void check(Point const a, Point const b, Point const c, Point const o, bool expected)
{
	...
	std::cout << color << o << " in " << a << b << c << ": " << verdict << " (expected " << expected << " got " << result << ')' << RESET << std::endl;
}
